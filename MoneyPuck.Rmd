...
## Multiple Regressions for Salary {.tabset .tabset-fade .tabset-pills}
...
As is eveident by the data, the only metrics that correlated with a Goalie's salary were Age and Games Started. No other hockey statistic registered as significant. To test whether this is a reasonable, or effective, way to pay Goalies, a round of multiple regressions was run with Win Percentage as the dependent variable, to see if any hockey statistic can be statistically shown to have a significant correlation with wins.

***

<br>

## Multiple Regressions for Win% {.tabset .tabset-fade .tabset-pills}
...
Reading the results for the above regressions, each statistic registered different levels of significance. GAA had a p-value of about .0001, meaning there was just about a 99.99% chance that that statistic is significant for wins. This is striking, as GAA did not register as significant for salary data. This was a common finding, indicating that contribution to wins did not necessarily translate into a higher salary. Each other statistic had varying p and t values. These results led to the creation of an algorithm that could serve as an index to model how efficiently a Goalie contributes to its team winning. This model was generated by multiplying the z-score for each statistic (no. of standard deviations above the league average) by the square of the average t-value for that same statistic. That number was then adjusted to produce a mean of zero.
